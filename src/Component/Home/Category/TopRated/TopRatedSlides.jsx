...
const TopRatedSlides = () => {
    ...
    // Filter data for items with a category greater than 7
    const topRatedData = slides.filter(item => parseFloat(item.rating) > 7)

    // Shuffle the filtered data
    const suffleData = [...topRatedData].sort(() => Math.random() - 0.5)
    return (
        <div>
            <Swiper
                //slidesPerView={3}
